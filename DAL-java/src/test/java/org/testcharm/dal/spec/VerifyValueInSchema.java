...
public class VerifyValueInSchema extends Base {
    ...
    void verify_type() {
        ...
        assertThat(dalException).hasMessage("Expected to match schema `MatchType` but was not\n" +
                "    Can not convert field `.value` (java.lang.String: invalid int) to type [org.testcharm.dal.format.Value]");
        ...
        assertThat(dalException).hasMessage("Expected to match schema `MatchType` but was not\n" +
                "    Can not convert null field `.value` to type [org.testcharm.dal.format.Value], use @AllowNull to verify nullable field");
    }
